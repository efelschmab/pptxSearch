...
def check_for_existing(file, time) -> int:
    ...
    sql_request = f"SELECT * FROM all_presentations WHERE presentation_name = '{file}' AND last_modified = '{time}'"
...
def query_database(query) -> list:
    ...
    sql_search_tables: str = f"SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%{query}%'"
    ...
    for table_name in table_names:
        ...
        sql_search_data: str = f"SELECT presentation_path, presentation_name, slide_number FROM {table_name[0]}"
